...
export default function MesReservationsPage() {
  ...
  useEffect(() => {
    ...
    try { if (storedUser) localUser = JSON.parse(storedUser); } catch {}
    ...
    if (activeUser?.email) {
      queryParams.set("email", activeUser.email);
    } else if (activeUser?.id || activeUser?.sub) {
      queryParams.set("user_id", String(activeUser.id || activeUser.sub));
    }

    fetch(`${API}/mes-reservations?${queryParams.toString()}`, { headers })
      .then((r) => r.json())
