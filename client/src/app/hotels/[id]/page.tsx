...
export default function HotelDetailPage() {
  ...
  // ── Normalize microservice hotel data → Laravel Hotel interface ───────────
  const normalizeHotel = (raw: any): Hotel => ({
    id: raw.id,
    nom: raw.nom,
    etoiles: raw.etoiles ?? 3,
    prix_par_nuit: raw.prix_par_nuit ?? raw.prix ?? 100,
    description: raw.description ?? null,
    image: raw.image ?? null,
    disponible: raw.disponible ?? true,
    destination: raw.destination
      ? { id: raw.destination.id, nom: raw.destination.nom, adresse: raw.destination.region }
      : null,
    chambres: Array.isArray(raw.chambres)
      ? raw.chambres.map((c: any) => ({
          id: c.id,
          type: c.type ?? c.nom ?? "double",
          nom: c.nom ?? "Chambre",
          prix_base_nuit: c.prix_base_nuit ?? c.prix ?? raw.prix_par_nuit ?? 100,
          capacite_adultes: c.capacite_adultes ?? c.capacite ?? 2,
  ...
  // ── Normalize avis — supporte {avis:[...], count, moyennes}, {data:[...]}, ou tableau direct ──
  const computeAvisMoyennes = (avis: AvisItem[]): AvisData["moyennes"] => {
    ...
    return {
      globale: avg(avis.map(a => a.note_globale)),
      qualite_prix: avg(avis.map(a => a.note_qualite_prix)),
      chambres: avg(avis.map(a => a.note_chambres)),
      emplacement: avg(avis.map(a => a.note_emplacement)),
      proprete: avg(avis.map(a => a.note_proprete)),
      services: avg(avis.map(a => a.note_services)),
      equipements: avg(avis.map(a => a.note_equipements)),
    };
  };

  const normalizeAvis = (raw: any): AvisData => {
    // Extraire le tableau d'avis selon le format reçu
    const list: any[] = Array.isArray(raw)
      ? raw
      : Array.isArray(raw?.avis)
        ? raw.avis
        : Array.isArray(raw?.data)
          ? raw.data
          : [];

    const avis: AvisItem[] = list.map((a: any) => ({
      id: a.id,
      hotel_id: a.hotel_id,
      user_id: a.user_id,
      user: a.user ?? { id: a.user_id, nom: "", prenom: "" },
      note_globale: a.note_globale ?? 0,
      note_qualite_prix: a.note_qualite_prix ?? null,
      note_chambres: a.note_chambres ?? null,
      note_emplacement: a.note_emplacement ?? null,
      note_proprete: a.note_proprete ?? null,
      note_services: a.note_services ?? null,
      note_equipements: a.note_equipements ?? null,
      commentaire: a.commentaire ?? null,
      created_at: a.created_at ?? new Date().toISOString(),
    }));

    // Si l'API fournit déjà les métadonnées agrégées (format Laravel), on les conserve.
    if (!Array.isArray(raw) && typeof raw?.count === "number") {
      return {
        count: raw.count,
        pct_recommande: raw.pct_recommande ?? 0,
        moyennes: raw.moyennes ?? computeAvisMoyennes(avis),
        avis,
      };
    }

    // Sinon on calcule les métadonnées depuis la liste d'avis.
    return {
      count: avis.length,
      pct_recommande: avis.length
        ? Math.round((avis.filter(a => a.note_globale >= 7).length / avis.length) * 100)
        : 0,
      moyennes: computeAvisMoyennes(avis),
      avis,
    };
  };
  ...
  // ── Fetch hotel ──────────────────────────────────────────────────────────
  const fetchHotel = () => {
    return fetch(`${API}/hotels/${id}`, { cache: "no-store" })
      .then(r => r.json())
      .then(raw => {
        // Le microservice retourne maintenant le même format que Laravel
        // (prix_base_nuit, capacite_adultes, pensions avec pivot.supplement_prix)
        // On normalise uniquement si ancien format simplifié sans prix_base_nuit
        const isFullFormat = raw?.chambres?.[0]?.prix_base_nuit !== undefined;
        setHotel(isFullFormat ? raw : normalizeHotel(raw));
        setLoading(false);
      })
  ...
  const fetchAvis = () => {
    setAvisLoading(true);
    fetch(`${API}/hotels/${id}/avis`, { cache: "no-store" })
      .then(r => r.json())
      .then(raw => {
        setAvisData(normalizeAvis(raw));
        setAvisLoading(false);
      })
      .catch(() => setAvisLoading(false));
  };
  ...
  const handleReservationClick = async (paymentType: "agence" | "deposit") => {
    ...
    try { if (userStr) localUser = JSON.parse(userStr); } catch {}

    const activeUser = localUser || nextAuthUser || null;
    const activeUserId = activeUser?.id || activeUser?.sub || 1;
    const activeUserNom = activeUser?.name || activeUser?.nom || (activeUser?.email ? activeUser.email.split('@')[0] : "Client TunisieBooking");
    const activeUserEmail = activeUser?.email || "client@gmail.com";
    ...
    try {
      const requests = rooms.map((room, idx) => {
        ...
        const roomTotal = chambre ? calculateChambreTotal(chambre, room) : grandTotal;
        ...
        return fetch(`${API}/bookings`, {
          ...
          body: JSON.stringify({
            ...
            user_nom: activeUserNom,
            user_email: activeUserEmail,
            hotel_id: +id,
            hotel_nom: hotel.nom,
            hotel_image: hotel.image,
            destination_nom: hotel.destination?.nom || "Tunisie",
            chambre_id: chambre ? +chambre.id : undefined,
            chambre_nom: chambre?.nom || "Chambre Standard",
            pension_id: pensionId ? +pensionId : undefined,
            pension_nom: pension?.nom || "Petit Déjeuner",
      ...
      if (responses.some(r => !r.ok)) {
        setError(results.find((r: any) => r?.message)?.message || "Erreur lors de la réservation.");
        return;
      }
