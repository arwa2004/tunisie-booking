...
async function getHotelsData(params: { destination_id?: string; etoiles?: string; prix_max?: string }) {
  ...
  try {
    // Construire les query params de filtre
    const qs = new URLSearchParams();
    if (params.destination_id) qs.set("destination_id", params.destination_id);
    if (params.etoiles)        qs.set("etoiles",        params.etoiles);
    if (params.prix_max)       qs.set("prix_max",        params.prix_max);

    const hotelsRes = await fetch(`${apiUrl}/hotels${qs.toString() ? "?" + qs.toString() : ""}`, { cache: "no-store" });
    if (hotelsRes.ok) {
      ...
      hotels = rawList.map((h: any, index: number) => ({
        ...
        etoiles: h.etoiles || 4,
        description: h.description || "Découvrez le luxe et le confort en Tunisie.",
        prix_par_nuit: h.prix_par_nuit || h.prixNuit || h.prix || 140,
        image: h.image || "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&auto=format&fit=crop&q=80",
        destination: h.destination || { id: 1, nom: h.ville || "Hammamet", region: "Tunisie" }
      }));
    }
  } catch (error) {
  ...
  try {
    // /api/destinations est proxied par Nginx vers hotel-service GET /destinations
