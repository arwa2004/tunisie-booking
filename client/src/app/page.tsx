...
async function getHomeData() {
  ...
  try {
    const hotelsRes = await fetch("http://127.0.0.1:8000/api/hotels/", { cache: "no-store" });
    if (hotelsRes.ok) {
      const data = await hotelsRes.json();
      const rawList = Array.isArray(data) ? data : (data.hotels || data.data || []);
      hotels = rawList.map((h: any, index: number) => ({
        id: h.id || (index + 1),
        nom: h.nom || "Hôtel TunisieBooking",
        etoiles: h.etoiles || 4,
        prix_par_nuit: h.prix_par_nuit || h.prixNuit || h.prix || 120,
        image: h.image || "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&auto=format&fit=crop&q=80",
        destination: h.destination || { id: 1, nom: h.ville || "Tunisie" }
      })).slice(0, 6);
    }
  } catch (error) {
...
function renderStars(nb: number) {
  return (
    <span className="text-[#85b919] text-sm tracking-tight">
      {"★".repeat(nb)}
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow">
        ...
        <section
          ...
          style={{
            backgroundImage:
              "linear-gradient(135deg, rgba(26,26,46,0.75), rgba(233,30,140,0.35)), url('https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=1400&auto=format&fit=crop&q=80')",
          }}
        >
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-3 drop-shadow-md animate-fade-in">
            Bienvenue sur <span className="text-[#e91e8c]">TunisieBooking</span>
          </h1>
        ...
        <section className="px-6 md:px-12 py-16 max-w-7xl mx-auto">
          <h2 className="text-3xl font-bold text-[#1a1a2e] mb-2">
            Nos <span className="text-[#85b919]">Destinations</span>
          </h2>
        ...
        <section className="px-6 md:px-12 py-16 max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10">
            <div>
              <h2 className="text-3xl font-bold text-[#1a1a2e] mb-2">
                Nos <span className="text-[#ec008c]">Bons Plans</span>
              </h2>
            ...
            <Link
              href="/hotels"
              className="hidden md:inline-block text-[#85b919] font-semibold text-sm hover:underline"
            >
                ...
                >
                  <div className="p-4 pb-3">
                    ...
                    <div className="flex items-end justify-between mt-1">
                      ...
                      <p className="text-[#85b919] font-extrabold text-xl leading-none">
                        {hotel.prix_par_nuit}
                        <span className="text-xs font-semibold ml-0.5 text-gray-600">DT</span>
                      </p>
                    </div>
          ...
          <div className="text-center mt-10 md:hidden">
            <Link
              href="/hotels"
              className="inline-block text-[#85b919] font-semibold text-sm hover:underline"
            >
              Voir tous les hôtels →
            </Link>
          </div>
        ...
        <section className="bg-[#0f0f1a] rounded-[30px] mx-4 sm:mx-8 px-6 md:px-12 py-16 text-white max-w-7xl md:mx-auto mb-16">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10">
            <div>
              <h2 className="text-3xl font-bold mb-2">
                Voyages <span className="text-[#85b919]">dans le Monde</span>
              </h2>
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-transparent to-transparent flex flex-col justify-end p-6">
                    <div className="flex justify-between items-end">
                      <div>
                        ...
                        <p className="text-[#ec008c] font-bold text-lg mt-1">
                          {voyage.prix} DT
                        </p>
                      </div>

                      <div className="bg-white text-[#ec008c] w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg opacity-0 transform -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 shadow-lg shadow-black/20">
                        ➔
                      </div>
                    </div>
                  </div>
          ...
          <div className="text-center mt-12">
            <Link
              href="/voyages"
              className="inline-block bg-[#e91e8c] hover:bg-[#c2185b] hover:shadow-xl hover:shadow-[#e91e8c]/35 text-white px-8 py-3 rounded-xl font-bold tracking-wide transition-all transform hover:-translate-y-[2px]"
            >
